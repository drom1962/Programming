...
void LOG::OpenOrCreateLog(wchar_t * Path)
	...
	if(Path!=nullptr)
		...
		swprintf_s(Buff,255,L"%s\%4d-%02d-%02d",Path,st.wYear,st.wMonth,m_Day);

		CreateDirectory(Buff,0);

		// Как то его добавим к имени лог
		swprintf_s(Buff,255,L"%s\%4d-%02d-%02d\\MediaServer.log",Path,st.wYear,st.wMonth,m_Day);
